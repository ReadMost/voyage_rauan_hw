...
tx = contract_factory.constructor(_greeting='rauan').buildTransaction(tx_args)
...
print("greeting: ", greeting) #hello rauan

# we can set a new name, though.
nonce = w3.eth.get_transaction_count(eoa.address)
tx = contract_instance.functions.setGreeting(_greeting="dake").buildTransaction({
    'from': eoa.address,
    'nonce': nonce,
    'gas': 888888,
    'maxFeePerGas': w3.toWei('2', 'gwei'),
